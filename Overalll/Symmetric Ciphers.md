...
- Server dùng AES-GCM với `KEY`, `IV` cố định.
- Associated data luôn là `b"CryptoHack"`.
- Endpoint `encrypt` trả `nonce`, `ciphertext`, `tag`, `associated_data`, nhưng nếu plaintext chứa `flag` thì chỉ trả `ciphertext`.
- Endpoint `decrypt` trả flag nếu plaintext hợp lệ chứa `b"give me the flag"`.

### Goal

- Forge một tag AES-GCM hợp lệ cho ciphertext của `b"give me the flag"` để gọi `decrypt` và lấy flag.

### Solution

- Do IV bị reuse, cùng AAD và cùng độ dài 1 block:

```text
tag1 xor tag2 = (c1 xor c2) * H^2
```

- Chọn 2 plaintext 16 byte là `00...00` và `01...01`, lấy `c1, tag1, c2, tag2`, rồi tính:

```text
H^2 = (tag1 xor tag2) / (c1 xor c2)
mask = tag1 xor c1 * H^2
```

- Sau đó encrypt `b"give me the flag"` để lấy ciphertext, tự tính tag:

```text
tag = ciphertext * H^2 xor mask
```

- Gửi `(nonce, ciphertext, tag, associated_data)` vào `decrypt`.
